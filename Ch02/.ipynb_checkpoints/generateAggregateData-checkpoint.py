...
#                                        'timestamp': [str(period_rng[times[n]].start_time + random_weekly_time_delta())]}))
        donations.append(pd.DataFrame({'user': [use_id],
                                       'amount': [donation_amounts[user_gives_idx + \
                                                  np.random.binomial(1, .3)]], 
                                       'timestamp': [str(period_rng[times[n]].start_time + random_weekly_time_delta())]}))
...
#                                     'emailsOpened': info}))
        emails.append(pd.DataFrame({'user': [idx] * len(info),
                            'week': [str(r.start_time) for r in user_rng],
                            'emailsOpened': info}))
...
#                                                     sum(info), idx, join_date.year))
        donations.append(produce_donations(user_rng, user_behaviors[idx],
                                           sum(info), idx, join_date.year))
...
emails = pd.concat(emails)

donations

donations = pd.concat([x for x in donations if len(x)>0])
